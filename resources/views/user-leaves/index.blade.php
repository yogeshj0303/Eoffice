    ...
    ($permissions['leave_delete'] == 2)
                              ...
                              <tr id="userRow{{ $leave->id }}">
                                          ...
                                          <td>{{ $leave->id }}</td>
                                            ...
                                            <td class="customer_name">{{ $leave->name }}</td>
                                            ...
                                            ->where('id',$leave->hod_verify_staff	)
...
@endif @endif </td>
                                            ...
                                            <?php
                                            if($leave->clerk_verify_staff != null  && $leave->clerk_otp_status == 0 ){
                                                
                                            $staff_name=DB::table('users')
                                            ->where('id',$leave->clerk_verify_staff	)
                                             ->leftJoin('departments', 'users.depart_id', '=', 'departments.id')
    ->leftJoin('designations', 'users.design_id', '=', 'designations.id')
    ->select(
        'users.*',
        ...
        'designations.designation_name'
                                            ...
                                            else if($leave->clerk_verify_staff != null  && $leave->clerk_otp_status == 1 ){
$staff_name = DB::table('users')
    ->where('users.id', $leave->frwd_hod_id)
    ->leftJoin('departments', 'users.depart_id', '=', 'departments.id')
    ->leftJoin('designations', 'users.design_id', '=', 'designations.id')
    ->select(
        'users.*',
    ...
    ->first();
                                                ...
                                                $staff_name=DB::table('users')->where('id',$leave->clerk_verify_staff	)
    ->leftJoin('departments', 'users.depart_id', '=', 'departments.id')
    ->leftJoin('designations', 'users.design_id', '=', 'designations.id')
    ->select(
        'users.*',
